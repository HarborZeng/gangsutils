...
trait PrintlnLogger extends BaseLogger {
  protected def printlnLog(msg: String, level: LogLevel.Value, dt: Boolean, trace: Boolean): Unit =
    buildLogContent(msg, level, dt, trace) |! { content =>
      level match {
